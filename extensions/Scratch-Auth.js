...
const authenticate = (thisObject, name) => {
    return new Promise(resolve => {
        const sanitizedName = encodeURIComponent(String(name).substring(0, 256).replace(/[^a-zA-Z0-9 _-]+/gmi, "_"));
        const waitingLink = `${window.location.origin}/wait.html`;
        const login = window.open(
            `https://auth.itinerary.eu.org/auth/?redirect=${btoa(waitingLink)}&name=${sanitizedName.length > 0 ? sanitizedName : "TurboWarp"}`,
        ...
        if (!login) {
            ...
            thisObject.isAppearing = false;
        } else {
        	thisObject.isAppearing = true;
        }
